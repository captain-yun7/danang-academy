...
// scripts/migrations/2026-07-04-exam-tweaks.sql 과 동일한 내용 — 파일은 문서/이력용, 실행은 이 배열.
// 멱등 SQL이므로 재실행 안전. neon http 드라이버는 statement 1개씩 실행.
// (이전 마이그레이션들은 이미 적용 완료 — 멱등이므로 필요 시 해당 파일/이력에서 재실행)
const STATEMENTS: string[] = [
  `alter table weekly_questions add column if not exists tts_speed numeric(3,2) not null default 0.5`,
];

...
const STATUS_BUTTONS: { value: Status; cls: string }[] = [
  { value: "present", cls: "bg-emerald-100 text-emerald-700 border-emerald-300" },
  { value: "late", cls: "bg-amber-100 text-amber-700 border-amber-300" },
  { value: "absent", cls: "bg-red-100 text-red-700 border-red-300" },
  { value: "excused", cls: "bg-blue-100 text-blue-700 border-blue-300" },
];
...
export function AttendanceBoard({
  ...
  if (roster.length === 0) {
    return <p className="text-sm text-[var(--color-muted)]">{t("rosterEmpty")}</p>;
  }
  return (
...
function AttendanceRow({
  ...
  const t = useTranslations("admin.sessions");
  const tBtn = useTranslations("admin.sessions.statusButtons");
    ...
    <li className="py-3">
      <div className="flex flex-wrap items-center gap-3">
        <div className="min-w-0 flex-1">
          ...
          {student.qr_scan_id && (
            <p className="text-[10px] text-emerald-600">{t("qrAutoTag")}</p>
          )}
        </div>

...
export function CommentEditor({
  ...
  function save() {
    ...
    startTransition(async () => {
      try {
        await saveComment({ roundId, studentId, comment: text });
        setSaved(text);
        setMsg("저장됨");
      } catch {
        setMsg("저장 실패");
  ...
  return (
    <div>
      <textarea
        ...
        rows={4}
        placeholder="‘코멘트 일괄 생성’으로 AI 초안을 만든 뒤 여기서 다듬으세요."
      ...
      <div className="mt-2 flex items-center justify-end gap-3 print:hidden">
        ...
        <button
          ...
          className="rounded-full border border-[var(--color-line)] px-4 py-1.5 text-sm font-semibold hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] disabled:opacity-50"
        >
          {pending ? "저장 중…" : "코멘트 저장"}
        </button>
      </div>

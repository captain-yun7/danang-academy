...
export function ResultClient({ id }: { id: string }) {
  const t = useTranslations("fpt.result");
  ...
  if (!data) {
    return <p className="mt-6 text-sm text-[var(--color-muted)]">{t("loading")}</p>;
  }
  ...
  if (data.status === "pending" || data.status === "processing") {
    return (
      <div className="mt-8 rounded-xl border border-[var(--color-line)] bg-white p-8 text-center">
        ...
        <p className="mt-4 text-sm font-semibold">{t("analyzing")}</p>
        <p className="mt-1 text-xs text-[var(--color-muted)]">{t("analyzingHint")}</p>
      </div>
    );
  }
  ...
  const levelKey = data.recommendedClassLevel as
    | "beginner" | "elementary" | "intermediate" | "advanced" | null;
  const level = levelKey ? tLevels(levelKey) : "—";
    ...
    <div className="mt-8 space-y-6">
      <div className="rounded-2xl border border-[var(--color-primary)]/30 bg-gradient-to-br from-[var(--color-soft)] to-white p-8">
        <p className="text-xs font-bold uppercase tracking-widest text-[var(--color-primary-deep)]">
          {t("scoreLabel", { name: data.visitorName })}
        </p>
        ...
        <p className="mt-3 text-sm">
          {t("recommendedClass")}: <strong className="text-base">{level}</strong>
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-[var(--color-line)] bg-white p-5">
          ...
          <p className="mt-2 text-sm leading-relaxed">{data.strengths || "—"}</p>
        </div>
        <div className="rounded-xl border border-[var(--color-line)] bg-white p-5">
          <p className="text-xs font-bold uppercase tracking-wider text-amber-600">
            {t("improvements")}
          </p>
          <p className="mt-2 text-sm leading-relaxed">{data.improvements || "—"}</p>
        </div>
      </div>
      ...
      {data.transcript && (
        <div className="rounded-xl border border-[var(--color-line)] bg-white p-5">
          ...
          <p className="mt-2 text-sm leading-relaxed">
            <span className="text-[var(--color-muted)]">{t("transcriptTarget")}:</span>{" "}
            {data.targetSentence}
            <br />
            <span className="text-[var(--color-muted)]">{t("transcriptHeard")}:</span>{" "}
            {data.transcript}
          </p>
        </div>
      )}
